...
///
/// All state is isolated to the main actor. CoreGraphics can invoke the
/// reconfiguration callback off-main, so the callback explicitly hops here.
@MainActor
final class TBVirtualDisplayModeMemory {
    ...
    static func preferenceKey(for identity: TBVirtualDisplayIdentity, receiverKey: String) -> String {
        "\(identity.productID)-\(identity.serialNumber)-\(receiverKey)"
    }
